...
export const EARLY_STOP_CONTINUE_NUDGE =
  'Continue the task now. Do not ask which files to review. If the original request is still ambiguous (stack, features, constraints), call the question tool; otherwise pick a reasonable default, keep using tools, and deliver. Only stop when the objective is done.';
