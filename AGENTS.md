...
- Tools operate on a per-workspace working tree, never the source directly. On first tool call, the agent lazy-copies the user's project into `<workspace>/.nanoagent/working-tree/` and the tools read/write there. Rollback: `/rollback` wipes the tree back to source; `/rollback <name>` restores a `/snapshot`. Snapshots live at `<workspace>/.nanoagent/snapshots/<name>.json` and capture every file that changed since the previous snapshot. When `cfg.workspace` IS the source (no separate project), the copy is shallow (top-level entries only) to avoid recursing into the tree being created.
