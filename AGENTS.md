...
├── snapshots.ts         # Named checkpoints + init baseline
...
- Tools operate on the user's project directory directly — `cfg.workspace` IS the directory the user pointed at, and tool reads/writes go there. `<workspace>/.nanoagent/` is this NanoAgent workspace's own harness state (sessions, worktree copies, snapshots) — part of this run, not an outside project folder and not a second workspace: init appends `.nanoagent/` to the project's `.gitignore`, discovery/`git_status` hide it, file tools and `change_workspace` block it, and the system prompt names it so the model must not explore, cd into, or commit it. On first agent init, a baseline snapshot of project files is captured at `<workspace>/.nanoagent/snapshots/init.json` (VCS/deps/caches such as `.git`, `node_modules`, `.pytest_cache` are skipped; unreadable directories are skipped instead of failing the capture). A watcher plus write/shell hooks then copy every touched file into `<workspace>/.nanoagent/worktree` (history only — tools do not redirect into that tree), save originals under `.nanoagent/history/originals`, and append `.nanoagent/history/journal.jsonl`. `/changes` lists that journal. `/rollback` (no name) restores from the baseline. Named snapshots live at `<workspace>/.nanoagent/snapshots/<name>.json` (one per `/snapshot` call) and capture diffs against the previous named snapshot. `/rollback <name>` walks the snapshot chain to compose deletions correctly. Rollback never deletes files inside skipped directories. Conversation history for the project is stored as 8-hex hashes in `<workspace>/.nanoagent/sessions`; boot with `nanoagent --resume HASH` (unique prefix ok) or list with `nanoagent --sessions` / `/sessions`. Matching older `$NANOAGENT_ROOT/sessions` files are migrated once.
