...
- Bun + OpenTUI agent; TUI code lives in `src/opentui/`; config at `<NANOAGENT_ROOT>/config/nanogent.json`, skills at `<NANOAGENT_ROOT>/skills/`, sessions at `<NANOAGENT_ROOT>/sessions/`, logs at `<NANOAGENT_ROOT>/logs/`. There is no `~/.nanogent.json` / `~/.nanoagent.json` / `~/.qwen-agent.json` / `<cwd>/.nanogent.json` / `<cwd>/.env` — every state file lives under the single canonical install root.
...
- Tools operate on the user's project directory directly — `cfg.workspace` IS the directory the user pointed at, and tool reads/writes go there. On first agent init, a baseline snapshot of project files is captured at `<workspace>/.nanoagent/snapshots/init.json` (VCS/deps/caches such as `.git`, `node_modules`, `.pytest_cache` are skipped; unreadable directories are skipped instead of failing the capture). `/rollback` (no name) restores from this baseline. Named snapshots live at `<workspace>/.nanoagent/snapshots/<name>.json` (one per `/snapshot` call) and capture diffs against the previous named snapshot. `/rollback <name>` walks the snapshot chain to compose deletions correctly. Rollback never deletes files inside skipped directories.
