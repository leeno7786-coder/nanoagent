...
- Tools operate on the user's project directory directly — `cfg.workspace` IS the directory the user pointed at, and tool reads/writes go there. On first agent init, a baseline snapshot of the workspace is captured at `<workspace>/.nanoagent/snapshots/init.json`; `/rollback` (no name) restores from this baseline. Named snapshots live at `<workspace>/.nanoagent/snapshots/<name>.json` (one per `/snapshot` call) and capture diffs against the previous named snapshot. `/rollback <name>` walks the snapshot chain to compose deletions correctly.
